...
export const dashboardApi = {
  getUpcomingQuestionnaires: async (
    filters: DashboardFilters = {}
  ): Promise<UpcomingQuestionnaireDto[]> => {
    ...
    const data = await httpClient.get<UpcomingQuestionnaireDto[]>(
      "/api/dashboard/questionnaires",
      {
        params,
      }
    );

    log.apiResponse("GET", "/api/dashboard/questionnaires", 200, {
      service: "dashboardApi",
...
export const getDashboardQuestionnaires = async (): Promise<
